...
GUARD="$(cd "$SP/partial" && PYTHONPATH="$OLDPWD" "$OLDPWD/.venv/bin/python" -c "import app.main" 2>&1)"
...
sh -n setup_env.sh 2>/dev/null && ok "setup_env.sh parses" || bad "setup_env.sh SYNTAX ERROR"
./place_call.sh 2>&1 | grep -q usage && ok "place_call.sh rejects no argument" || bad "place_call.sh accepts no argument"
./place_call.sh 9731130674 2>&1 | grep -q Refusing && ok "place_call.sh rejects non-E.164" || bad "place_call.sh accepts bad number"
